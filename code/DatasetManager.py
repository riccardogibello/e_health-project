...
class DatasetManager:
    def __init__(self, dataset_file):
        print(f'Dataset Manager started in thread : {threading.currentThread()}')
    ...
    def load_apps_into_db(self, filter_categories=SERIOUS_GAMES_CATEGORIES_LIST, filter_columns=True):
        ...
        if DEBUG:
            print(f'Start loading into database data from dataset {self.__dataset_file}')
    ...
    def store_app_data(self, app_index):
        ...
        insert_stmt = (
            "INSERT IGNORE INTO APP(app_name, category, app_id)"
            "VALUES (%s, %s, %s)"
        )
